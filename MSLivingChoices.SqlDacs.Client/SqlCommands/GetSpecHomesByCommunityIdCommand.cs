...
{
	internal class GetSpecHomesByCommunityIdCommand : CachedBaseCommand<List<SpecHome>>
	{
		private readonly long _communityId;

		private const CommunityUnitType UnitType = CommunityUnitType.SpecHome;

		private readonly List<SpecHome> _specHomes;
		...
		protected override void CommandBody(SqlCommand command)
		{
			...
			{
				SpecHome specHome = sqlDataReader.GetSpecHome();
				this._specHomes.Add(specHome);
			}
			if (sqlDataReader.NextResult())
			{
				while (sqlDataReader.Read())
				{
					...
					this._specHomes.First<SpecHome>((SpecHome x) => x.Id == value).Amenities.Add(valueOrDefault);
				}
			}
			if (sqlDataReader.NextResult())
			{
				...
				{
					this._specHomes.First<SpecHome>((SpecHome x) => x.Id == key).Images = nums[key];
				}
			}
			if (sqlDataReader.NextResult())
			{
				while (sqlDataReader.Read())
				{
					...
					this._specHomes.First<SpecHome>((SpecHome x) => x.Id == value1).Coupon = coupon;
				}
			}
		}

		protected override List<SpecHome> GetCommandResult(SqlCommand command)
		{
			return this._specHomes;
		}
	}
}