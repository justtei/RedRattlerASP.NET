	...
	public static class MslcUrlBuilder
	{
		public static string BaseUrl
		{
			get
			{
				...
				{
					Uri url = HttpContext.get_Current().get_Request().get_Url();
		...
		{
			VirtualPathData virtualPathData = RouteTable.get_Routes().GetVirtualPath(null, routeName, new RouteValueDictionary(routeValues));
			return ((virtualPathData != null ? string.Concat(MslcUrlBuilder.BaseUrl, virtualPathData.get_VirtualPath()) : string.Empty)).ToLower();
		}
	}
}