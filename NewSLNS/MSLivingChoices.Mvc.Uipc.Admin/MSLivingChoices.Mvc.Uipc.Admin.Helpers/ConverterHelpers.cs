	...
	public static class ConverterHelpers
	{
		...
		public static List<SelectListItem> CountryListToSelectListItems(IEnumerable<Country> countryList, string selectedCountryCode)
		{
			...
			foreach (Country country in countryList)
			{
				SelectListItem item = new SelectListItem()
				{
					Text = country.Name,
					Value = country.Code,
					Selected = (selectedCountryCode == null ? false : country.Code == selectedCountryCode)
				};
				result.Add(item);
			}
			return result;
		...
		where T : struct
		{
			...
			return ConverterHelpers.EnumToList<T>().Select<T, SelectListItem>((T enumItem) => {
				...
				{
					Text = enumItem.GetEnumLocalizedValue<T>() ?? enumItemString,
					Value = enumItemString,
					Selected = (!selectedItem.HasValue ? false : enumItem.Equals(selectedItem.Value))
				};
			}).ToList<SelectListItem>();
		}
	}
