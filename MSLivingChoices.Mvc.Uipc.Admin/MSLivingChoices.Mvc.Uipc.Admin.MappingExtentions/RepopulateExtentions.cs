...
namespace MSLivingChoices.Mvc.Uipc.Admin.MappingExtentions
{
	public static class RepopulateExtentions
	{
		public static NewCommunityVm Repopulate(this NewCommunityVm viewModel)
		{
			...
			viewModel.PhoneList.AdditionalPhones.ForEach((PhoneVm ph) => ph.PhoneTypes.RemoveAll((SelectListItem pt) => pt.Text.Contains("Provision")));
		...
		public static NewServiceProviderVm Repopulate(this NewServiceProviderVm model)
		{
			...
			model.PhoneList.AdditionalPhones.ForEach((PhoneVm ph) => ph.PhoneTypes.RemoveAll((SelectListItem pt) => pt.Text.Contains("Provision")));
		...
		public static AddressVm Repopulate(this AddressVm address)
		{
			...
			address.Country.AvailableCountries = countries.ConvertAll<SelectListItem>((Country m) => {
				...
				long? id = m.Id;
				selectListItem.Value = id.ToString();
				selectListItem.Text = m.Name;
				id = m.Id;
				long? nullable = address.Country.Id;
				selectListItem.Selected = id.GetValueOrDefault() == nullable.GetValueOrDefault() & id.HasValue == nullable.HasValue;
